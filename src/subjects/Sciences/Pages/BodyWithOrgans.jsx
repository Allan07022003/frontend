  ...
  const [{ isOver }, drop] = useDrop(() => ({
    ...
    drop: (item) => {
      ...
      if (item.name === accept) {
        ...
        speak(
          `Intenta colocar el órgano correcto. ${getArticle(wrongOrganNameInSpanish, isWrongOrganPlural)} ${wrongOrganNameInSpanish} no ${getVerb(isWrongOrganPlural)} aquí.`
        );
      }
    },
    collect: (monitor) => ({
...
const BodyWithOrgans = () => {
  ...
  // Función para reiniciar los órganos
  const resetOrgans = () => {
    setOrgansPlaced({
      corazon: false,
      pulmones: false,
  ...
  return (
    <DndProvider backend={isMobile ? TouchBackend : HTML5Backend}>
      <Header
        ...
        showResetButton={true} // Mostrar el botón de reiniciar solo en esta actividad
        onReset={resetOrgans} // Función que reinicia los órganos
      />
